...
resource "aws_ec2_tag" "eks_subnet_tag_public_elb" {
  for_each    = toset(data.terraform_remote_state.customer-vpc.outputs.public_subnet_ids)
...
resource "aws_ec2_tag" "eks_subnet_tag_private_elb" {
  for_each    = toset(data.terraform_remote_state.customer-vpc.outputs.private_subnet_ids)
  resource_id = each.value
